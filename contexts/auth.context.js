...
const AuthProvider = ({ children }) => {
    ...
    const [eventTutorialDone, setEventTutorialDone] = useState(true); // the bool which checks if tutorial needed when user click on event on main page
    const [peopleTutorialDone, setPeopleTutorialDone] = useState(true); // the bool which checks if tutorial needed when user click on person small card on main page
    const [eventCardTutorialDone, setEventCardTutorialDone] = useState(true); // the bool which checks if the event ad is needed when user is swiping other users' cards
    const [mySchoolCardTutorialDone, setMySchoolCardTutorialDone] =
        useState(true); // the bool which checks if the My School ad is needed when user is swiping other users' cards
    const [campusGhostCardTutorialDone, setCampusGhostCardTutorialDone] =
        useState(true); // the bool which checks if the Campus Ghost ad is needed when user is swiping other users' cards
    const [mainPageTutorialDone, setMainPageTutorialDone] = useState(true);
        ...
        await axios
            ...
            .catch(async (error) => {
                ...
                if (counter < 10 && error?.response?.status !== 400) {
                    ...
                    Alert.alert("Hata", error?.responsre?.data, [
                        { text: "Kontrol Edeyim" },
                    ]);
    ...
    const signOut = async () => {
        try {
            ...
            axios
                .post(
                    "https://devmessage.meetdorm.com/registerToken",
                    encryptedToken,
                    {
