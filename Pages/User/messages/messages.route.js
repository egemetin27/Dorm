...
const Messages = ({ navigation, route }) => {
    ...
    const [chatMode, setChatMode] = useState(
        route.params?.matchMode ?? (user.matchMode || 0)
    );
    ...
    const unreadInFriend = matchesList["1"].nonEmptyChats.some(
        ({ MatchId }) => {
            return unreadChatIDS.includes(MatchId.toString());
        }
    );
    ...
    useEffect(() => {
        ...
        sortedList.sort((a, b) => {
            return getLastMessage(a.MatchId).date <
                getLastMessage(b.MatchId).date
                ? 1
                : -1;
        });
        ...
        <View style={styles.container}>
            <View style={styles.header}>
                ...
                <Switch
                    choiceList={[
                        `Flört Modu ${
                            chatMode === 1 && unreadInFlirt ? "*" : ""
                        }`,
                        `Arkadaş Modu  ${
                            chatMode === 0 && unreadInFriend ? "*" : ""
                        }`,
                    ]}
                    choice={chatMode}
                ...
                ListEmptyComponent={() => (
                    ...
                    >
                        <Text
                            ...
                            style={{ textAlign: "center", color: "#9D9D9D" }}
                        >
                            Keşfetmeye Başla. Ana sayfaya giderek diğer
                            kullanıcılarla eşleştiğinde buradan onlara mesaj
                            atabileceksin. Sana mesaj atmak isteyen bir sürü
                            kişi var, sadece senin kaydırmanı bekliyorlar.
                        </Text>
                    </View>
                )}
                ItemSeparatorComponent={() => (
