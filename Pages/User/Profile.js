...
export default function Profile({ navigation }) {
    ...
    useEffect(() => {
        ...
        const profile = async () => {
            try {
                ...
                setSex(
                    user.Gender == "null" ? "" : lists.genderList[user.Gender]
                );
                setInterestedSex(
                    user.InterestedSex == "null"
                        ? ""
                        : lists.interestedSexList[user.InterestedSex]
                );
                setExpectation(
                    user.Expectation == "null"
                        ? ""
                        : lists.expectationList[user.Expectation]
                );
                ...
                setDiet(
                    user.Beslenme == "null" ? "" : lists.dietList[user.Beslenme]
                );
                setDrink(
                    user.Alkol == "null"
                        ? ""
                        : lists.smokeAndDrinkList[user.Alkol]
                );
                ...
                setSmoke(
                    user.Sigara == "null"
                        ? ""
                        : lists.smokeAndDrinkList[user.Sigara]
                );
    ...
    const handleEdit = (label, visible) => {
        ...
        } else if (label === "Hakkımda")
            navigation.push("ProfileEditPage", { field: "Hakkımda" });
        else if (label !== "") {
            if (visible === "multiple") {
    ...
    const handleSave = async () => {
        const lName =
            name.trim().lastIndexOf(" ") == -1
                ? ""
                : name.trim().slice(name.trim().lastIndexOf(" ") + 1);
            ...
            >
                ...
                >
                    ...
                    <View style={{ paddingHorizontal: width * 0.047 }}>
                        ...
                        <View>
                            {FIELDS.map(
                                ({ label, IconTextList, visible }, index) => {
                                    return (
                                        <Pressable
                                            onPress={
                                                isEditable
