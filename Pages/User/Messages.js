			...
			>
					...
					{chatMod[0] == 1 ? (
						...
						>
							<View flexDirection = "row">
								<Text style={{ fontSize: 20, color: colors.cool_gray, paddingRight: 5 }}>Flört Modu</Text>
								{ unreadMsgInFlirt == 1 ?
									(
										<LinearGradient
				...
				<View style={{ flex: 1 }}>
					{chatMod[1] == 1 ? (
						...
						>
							<View flexDirection = "row">
								<Text style={{ fontSize: 20, color: colors.cool_gray, paddingRight: 5 }}>Arkadaş Modu</Text>
								{ unreadMsgInFriend == 1 ?
									(
										<LinearGradient
											colors={["#4136F1", "#8743FF"]}
						...
						<View >
							...
							<FlatList
								...
								renderItem={({ item, index }) => {
									if (item.mod == 1 && item.lastMsg != null && item.firstUser.id == myUserID && item.status == "Active" && item.unreadMsg != 0 && item.lastMsgSender != myUserID) {
										setUnreadMsgInFriend(1);
									}
									if (item.mod == 1 && item.lastMsg != null && item.secondUser.id == myUserID && item.status == "Active" && item.unreadMsg != 0 && item.lastMsgSender != myUserID) {
										setUnreadMsgInFriend(1);
									}
					...
					<View>
						...
						<View >
							...
							<FlatList
								...
								data={chatRooms}
								renderItem={({ item, index }) => {
									if (item.mod == 0 && item.lastMsg != null && item.firstUser.id == myUserID && item.status == "Active" && item.unreadMsg != 0 && item.lastMsgSender != myUserID) {
									...
									if (item.mod == 0 && item.lastMsg != null && item.secondUser.id == myUserID && item.status == "Active" && item.unreadMsg != 0 && item.lastMsgSender != myUserID) {
										console.log(item.lastMsg);
										console.log(item.id);
										console.log(item.unreadMsg);
										setUnreadMsgInFlirt(1);
