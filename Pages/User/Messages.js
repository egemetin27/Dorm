				...
				<View style = {{flex: 1}}>
					{
						chatMod[0] == 1 ? (
							...
							<Pressable style = {{alignItems: "center", justifyContent: "center"}} onPress={() => {setChatMod([1, 0]);}}>
				...
				<View style = {{flex: 1}}>
					{
						chatMod[1] == 1 ? (
							...
							<Pressable style = {{alignItems: "center", justifyContent: "center"}} onPress={() => {setChatMod([0, 1]);}}>
									<Text style={{fontSize: 20, color: colors.cool_gray}}>
										Arkadaş Modu
								</Text>
							</Pressable>
						...
						<View>
							...
							<View>
								<FlatList
									...
									renderItem={({item,index}) => {
										if (item.mod == 0 && item.lastMsg != null && item.firstUser.id == myUserID) {
											return <MsgBox data= {item.secondUser} lastMsg = {item.lastMsg} lastTime = {item.updatedAt} handleDelete = {() => deleteItem(index)} openChat = {() => openChat(item.secondUser, myUserID, item.id)} userID = {myUserID}/>;
										}
										if (item.mod == 0 && item.lastMsg != null && item.secondUser.id == myUserID) {
											return <MsgBox data= {item.firstUser} lastMsg = {item.lastMsg} lastTime = {item.updatedAt} handleDelete = {() => deleteItem(index)} openChat = {() => openChat(item.firstUser, myUserID, item.id)} userID = {myUserID}/>;
										}
									}}
								/>
							</View>
