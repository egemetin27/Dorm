...
export default function EventList({ navigation }) {
    ...
    const [shouldScrollTopOnBackButton, setShouldScrollTopOnBackButton] =
        useState(false);
    ...
    const animatedHeader = useAnimatedStyle(() => {
        ...
        return {
            elevation: withTiming(
                interpolate(categoryRow.value, [-1, 1], [16, 0])
            ),
            shadowOpacity: withTiming(
                interpolate(categoryRow.value, [-1, 1], [1, 0])
            ),
        };
    }, []);
    ...
    return (
        ...
        >
            ...
            <FlatList
                ...
                ListFooterComponent={() =>
                    shownEvents.length === 0 ? <ListEmptyComponent /> : null
                }
            />
        </View>
    );
